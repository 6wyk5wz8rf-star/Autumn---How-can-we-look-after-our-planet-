...
function renderGuidedActivityGuide(activity) {
  if (!activity) return '';
  return `<section class="science-guided-guide" aria-label="Guided scientific pathway"><article><p class="eyebrow">Invitation</p><p>${escapeHTML(activity.flow.notice.prompt)}</p></article><article><p class="eyebrow">Make or decide</p><p>${escapeHTML(activity.flow.make.product)}</p></article><details><summary>Optional Key Check · unscored</summary><p>${escapeHTML(activity.keyCheck.prompt)}</p><p class="small muted">The check names the scientific reasoning. It has no grade, percentage, pass or fail.</p></details></section>`;
}

const SCIENCE_PRIMARY_MODES = Object.freeze([
  { id: 'observe', title: 'Observe', mark: '◉', toolIds: ['organism-browser', 'observation-lens', 'compare-organisms'] },
  { id: 'group', title: 'Group', mark: '∩', toolIds: ['free-sorting', 'group-rule-tester', 'backbone-explorer', 'vertebrate-gallery', 'invertebrate-gallery'] },
  { id: 'classify', title: 'Classify', mark: '⑂', toolIds: ['follow-classification-key', 'build-classification-key', 'repair-broken-key', 'mystery-organism'] },
  { id: 'habitats', title: 'Habitats', mark: '▧', toolIds: ['habitat-windows', 'microhabitat-lens', 'habitat-builder', 'change-laboratory', 'survey-builder', 'create-science-challenge'] },
]);

function sciencePrimaryMode(toolId) {
  return SCIENCE_PRIMARY_MODES.find(({ toolIds }) => toolIds.includes(toolId)) || SCIENCE_PRIMARY_MODES[0];
}

function renderScienceModes(tool) {
  const current = sciencePrimaryMode(tool.id);
  return `<nav class="destination-mode-nav no-print" aria-label="Living Things Observatory modes">${SCIENCE_PRIMARY_MODES.map((mode) => `<button type="button" data-route="science-tool" data-route-value="${mode.toolIds[0]}" aria-current="${mode.id === current.id ? 'page' : 'false'}"><span aria-hidden="true">${mode.mark}</span><strong>${mode.title}</strong></button>`).join('')}</nav><details class="destination-more-tools no-print"><summary>More tools</summary><div>${current.toolIds.map((toolId) => { const item = getScienceTool(toolId); return `<button type="button" data-route="science-tool" data-route-value="${item.id}" aria-current="${item.id === tool.id ? 'true' : 'false'}"><strong>${escapeHTML(item.title)}</strong><span>${escapeHTML(item.invitation)}</span></button>`; }).join('')}</div></details>`;
}
...
export class LivingThingsObservatory {
  constructor(host, { toolId = null, activity = null, savedState = null, scaffold = 'core', returnToContext = false, onChange, onSave, onSpeak, onRecord, onToast } = {}) {
    ...
    this.host = host;
    this.tool = getScienceTool(activity?.toolId || toolId || 'organism-browser') || getScienceTool('organism-browser');
  ...
  render() {
    ...
    this.host.innerHTML=`<section class="page living-things-tool" aria-labelledby="science-tool-title"><header class="science-tool-head no-print">${returnControl}<div><p class="eyebrow">${escapeHTML(this.activity?'Guided Key Activity':'Living Things Observatory · open exploration')}</p><h1 id="science-tool-title">${escapeHTML(this.activity?.title||'Living Things Observatory')}</h1><p>${escapeHTML(this.activity?.shortInvitation||this.tool.invitation)}</p>${this.activity?`<p class="science-objective"><strong>Scientific purpose:</strong> ${escapeHTML(this.activity.curriculumObjective)}</p>`:''}</div><div class="science-tool-actions"><span class="small muted">Draft saves quietly</span><button class="button secondary" type="button" data-science-action="open-board">Board View</button></div></header>${renderScienceModes(this.tool)}<div class="destination-tool-title"><p class="eyebrow">${escapeHTML(sciencePrimaryMode(this.tool.id).title)}</p><h2>${escapeHTML(this.tool.title)}</h2></div>${renderGuidedActivityGuide(this.activity)}${this.scaffold==='intensive'?'<div class="science-scaffold-note"><strong>One decision at a time.</strong> The organism set is smaller; the scientific objective is unchanged.</div>':''}${renderToolModel(this.state,this.tool,this.activity)}<details class="show-me no-print"><summary>Show me</summary><div><p><strong>Watch:</strong> Choose one visible feature and point to the evidence.</p><p><strong>Example:</strong> “I observed six legs. I infer this may be an insect.”</p><p><strong>Your turn:</strong> Return to the current organism and add one observation before an inference.</p></div></details><footer class="science-save-bar no-print"><div><strong>Keep this thinking?</strong><span>Organism IDs, branch logic, source metadata and the generator seed stay with the work.</span></div><button class="button secondary" type="button" data-science-action="undo-science" ${this.history.length?'':'disabled'}>Undo</button><button class="button secondary" type="button" data-science-action="redo-science" ${this.future.length?'':'disabled'}>Redo</button><button class="button secondary" type="button" data-science-action="new-task">Try another set</button><button class="button secondary" type="button" data-science-action="record-science">Record voice</button><span data-audio-recorder-status class="small muted">Optional</span><button class="button" type="button" data-science-action="save-science">Keep in My Work</button><details class="quiet-actions"><summary>More</summary><button class="button secondary" type="button" data-action="print-page">Print</button></details></footer></section>`;
  }
